CREATE  PROCEDURE [dbo].[ProcessWBEGLogin]
	@UID	varchar(255) 
AS
...
SELECT @UID = UniqueID, @UserName = UserName, @EMail = EMail, @LogoutURL = LogoutURL FROM WBEG WHERE Convert(varchar(255), UniqueID) = @UID
SET @Row = @@RowCount
