CREATE PROCEDURE dbo.ProcessASIAMLogin
	@UID	varchar(255) 
AS
...
SELECT @UID = UniqueID, @UserName = UserName, @EMail = EMail, @LogoutURL = LogoutURL FROM ASIAM WHERE Convert(varchar(255), UniqueID) = @UID
SET @Row = @@RowCount
DELETE ASIAM WHERE  Convert(varchar(255), UniqueID) = @UID
